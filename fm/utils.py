...
# Be careful about dimentionality -- maybe tf.transpose(outputs) is needed

def loss_truncated_logistic(outputs, y):
...
def loss_cross_entropy(y, outputs):
    return tf.nn.weighted_cross_entropy_with_logits(targets=y, logits=tf.reshape(outputs, [-1]), pos_weight=5)
...
def rank(outputs, i, num_sample):
    b = num_sample+1-rankdata(outputs, method='max')   # rank data default ascending
    return b[i], 1. / b[i], int(b[i] == 1), int(b[i] <= 3), int(b[i] <= 10)

def save_obj(dic, name):
    with open('./dictionary/' + name + '.pkl', 'wb') as f:
        pickle.dump(dic, f, pickle.HIGHEST_PROTOCOL)

def load_obj(name):
    with open('./dictionary/' + name + '.pkl', 'rb') as f:
